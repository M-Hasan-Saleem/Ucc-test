...
export default function AllUsers() {
  ...
  const getAllUsers = async () => {
    let response = await axios.get(`${baseUrl}/posts/getAllPostsByAdmin?limit=60&page=0`, {
  ...
  const requestForChangeStatus = async(id, status) => {
    let response = await axios.put(`${baseUrl}/posts/updatePostStatus/${id}`, {status: status}, {
      headers: { Authorization: `Bearer ${AdminData?.JWT_TOKEN}` },
    });
    let data = response.data;
    console.log(data);
                            ...
                            {AllUsers?.map((v, i) => {
                              return (
                                <tr key={i}>
                                  ...
                                  <td>{
                                    v?.status == 1 ?
                                      <button onClick={() => requestForChangeStatus(v?._id, false)}>Active</button>
                                      :
                                      <button onClick={() => requestForChangeStatus(v?._id, true)}>Non Active</button>
                                  }</td>
                                </tr>
                              );
                            })}
